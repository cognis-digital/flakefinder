...
**flakefinder** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `flakefinder` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
flakefinder ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
flakefinder ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
flakefinder ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `flakefinder` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="flakefinder") for rec in records]   # records = your JSON output
