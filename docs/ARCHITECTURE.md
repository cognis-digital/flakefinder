...
- **rules/analyzers** apply the heuristics shipped in `flakefinder/core.py`.
- **score** ranks by severity.
- **MCP server** (`flakefinder mcp`) exposes `scan` for Cognis.Studio agents.
