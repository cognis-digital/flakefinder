"""Native cognis-connect emit for flakefinder — forward findings to any platform.

Maps flakefinder's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    flakefinder ... --format json | flakefinder-emit --to stix
    flakefinder-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('test_id', '')
        out['severity'] = 'medium'
        out['type'] = 'flaky-test'
        out['description'] = f"Flaky test detected in CI history: {rec.get('test_id', '')}"
        out['tags'] = ['ci', 'flaky']
